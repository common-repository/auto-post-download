...
class APD_Attatchment {
	...
	private function generateZipFile(){
        $result = $this->zipArchive->open($this->zipFile, ZipArchive::CREATE|ZipArchive::OVERWRITE);
		if (!$result){
			die("Failed to create archive. Error code {$result}\n");
		}        
	...
	private function generateAttachment(){
		if(get_post_meta($this->postId, 'adp_attachment_id', true) !== null){
			wp_delete_attachment(get_post_meta($this->postId, 'adp_attachment_id', true));
		}
        ...
        if (!add_post_meta($this->postId, 'adp_attachment_id', $attachId, true)) { 
           update_post_meta($this->postId, 'adp_attachment_id', $attachId);
        }        
	}
